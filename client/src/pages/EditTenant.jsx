...
const EditTenant = () => {
  ...
  console.log("floorNo number", floorNo);
  // console.log("Flat number", flatNo);
  useEffect(() => {
    const getDetails = async () => {
      try {
        ...
        console.log("checking", respond.data.flatNo);
        setclientName(respond.data.clientName);
        setadhaarcardno(respond.data.adhaarcardno);
        setFloorNo(respond.data.floorNo);
        setFlatNo(respond.data.flatNo);
        setDepositPaid(respond.data.DepositPaid);
        setAmountPaid(respond.data.AmountPaid);
        setDateTo(respond.data.DateTo);
        setDateFrom(respond.data.DateFrom);
        setPhoneNo(respond.data.PhoneNo);
        setIsNew(respond.data.IsNew);
        setRemainingAmount(respond.data.RemainingAmount);
        setnoOfSharing(respond.data.noOfSharing);
        setRentPaid(respond.data.RentPaid);
        setStatus(respond.data.Status);
        setSharing1(respond.data.sharing1);
        setSharing2(respond.data.sharing2);
        setSharing3(respond.data.sharing3);
        setSharing4(respond.data.sharing4);
        setSharing5(respond.data.sharing5);
        setDeposit(respond.data.deposit);
      } catch (error) {
        console.log(error);
      }
    };
  ...
  useEffect(() => {
    ...
    const newrentamount = newRentamount(noOfSharing);
  ...
  const newRentamount = (getsharing) => {
    let newRemainingAmount = 0;
    let chooseoption = getsharing;
  ...
  useEffect(() => {
    const handlefFloorChange = () => {
      const selectedFloor = floorNo;
      ...
      <div>
        ...
        >
          ...
          <input
            type="text"
            name="RentAmount"
            value={newRentamount(noOfSharing)}
            required
